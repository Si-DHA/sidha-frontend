...
export const getWeeklyTotalNewClientnMonth = async (month: Number, year: Number): Promise<any> => {
  try {
    const response = await fetch(BASE_URL + `/user/client/total/weekly?month=${month}&year=${year}`, {
      method: 'GET',
      headers: {
    ...
    const options = {
      chart: {
        title: `Jumlah Klien Baru per Minggu pada ${monthName} ${year}`,
