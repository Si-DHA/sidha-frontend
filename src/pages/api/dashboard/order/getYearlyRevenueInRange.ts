...
export const getYearlyRevenueInRange = async (startYear: Number, endYear: Number): Promise<any> => {
  try {
    const response = await fetch(BASE_URL + `/order-item/revenue/yearly?startYear=${startYear}&endYear=${endYear}`, {
      method: 'GET',
      headers: {
    ...
    const options = {
      chart: {
        title: `Jumlah Pendapatan Tahun ${startYear}-${endYear}`,
